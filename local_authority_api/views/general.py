from flask import request, Blueprint, Response, current_app, g
from local_authority_api.dependencies import postgres
import datetime
import json
        ...
        for dependency, value in current_app.config.get("DEPENDENCIES").items():
            ...
            if "postgres" in value:
                ...
                try:
                    ...
                    db["current_timestamp"] = db_timestamp.strftime(
                        "%Y-%m-%dT%H:%M:%S.%f")[:-3] + 'Z'  # trim microseconds to 3 to match java
                    db["status"] = "OK"
                except Exception as e:
                    current_app.logger.error(
                        "Unknown error occurred during health cascade on request to database: {};"
